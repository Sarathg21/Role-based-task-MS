...
const RecurringTasksPage = () => {
    ...
    const fetchTemplates = async () => {
        setLoading(true);
        try {
            // Fetching tasks with is_recurring=true
            const res = await api.get('/tasks', { params: { is_recurring: true } });
    ...
    const handleToggleStatus = async (template) => {
        ...
        try {
            // Assuming endpoint for internal status toggle or patch
            await api.patch(`/tasks/${template.id}`, { is_active: newStatus });
            setTemplates(prev => prev.map(t => t.id === template.id ? { ...t, is_active: newStatus } : t));
            toast.success(`Task ${newStatus ? 'activated' : 'paused'} successfully`);
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] p-6 font-['Aptos']">
            {/* Header Area */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 font-bold uppercase tracking-[0.3em] text-[10px] ml-11">Governance & Automated Directives</p>
                    </div>
                </div>

                <button 
                    onClick={() => navigate('/tasks/assign')}
                    className="bg-violet-600 hover:bg-violet-700 text-white px-8 py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl shadow-violet-200 transition-all flex items-center gap-3 active:scale-95 self-start md:self-center"
                >
            ...
            <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden min-h-[600px] flex flex-col">
                <div className="px-10 py-8 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-sm font-black text-slate-800 uppercase tracking-tight italic">Active Governance Templates</h2>
                    </div>
                    <div className="flex items-center gap-6">
                        <div className="text-center">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Templates</p>
                            <span className="text-xl font-black text-slate-900 tabular-nums">{templates.length}</span>
                        </div>
                        <div className="w-px h-8 bg-slate-200" />
                        <div className="text-center">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Enabled</p>
                ...
                <div className="flex-1">
                    {loading ? (
                        <div className="h-[400px] flex flex-col items-center justify-center gap-4">
                            ...
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Hydrating Directive Registry</p>
                        </div>
                    ) : templates.length === 0 ? (
                            ...
                            <table className="w-full text-left">
                                <thead className="bg-slate-50/30">
                                    <tr className="border-b border-slate-100">
                                        <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Directive / Purpose</th>
                                        <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-center">Frequency</th>
                                        <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-center">Owner</th>
                                        <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-center">Next Run Date</th>
                                        <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-right">Governance</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-50">
                                    {templates.map((template) => (
                                        <tr key={template.id} className="group hover:bg-slate-50/50 transition-all duration-300">
                                            <td className="px-10 py-6">
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm border border-white transition-transform group-hover:scale-110 ${template.is_active !== false ? 'bg-violet-100 text-violet-600' : 'bg-slate-100 text-slate-400'}`}>
                                                        <RefreshCw size={20} />
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-black text-slate-800 leading-tight mb-1 group-hover:text-violet-600 transition-colors uppercase tracking-tight">{template.title}</p>
                                            ...
                                            <td className="px-6 py-6 border-x border-slate-50/50">
                                                <div className="flex flex-col items-center gap-1">
                                                    <span className="px-3 py-1 rounded-full bg-blue-100 text-blue-700 text-[9px] font-black uppercase tracking-widest border border-blue-200">
                                                        {template.recurring_frequency}
                                                    </span>
                                                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest italic">Day {template.recurring_day || 'N/A'}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-6 text-center">
                                                <div className="flex items-center justify-center gap-2 bg-slate-50 border border-slate-200 px-4 py-2 rounded-xl inline-flex shadow-inner">
                                                    ...
                                                    <span className="text-[11px] font-black text-slate-700 tabular-nums">{getNextRunDate(template)}</span>
                                                </div>
                                            </td>
                                            <td className="px-10 py-6 text-right">
                                                <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity translate-x-2 group-hover:translate-x-0">
                                                    ...
                                                    <button 
                                                        ...
                                                        disabled={actionLoading === template.id}
                                                        className={`p-2.5 rounded-xl border transition-all ${template.is_active !== false 
                                                            ? 'bg-amber-50 border-amber-200 text-amber-600 hover:bg-amber-100' 
                                                            : 'bg-emerald-50 border-emerald-200 text-emerald-600 hover:bg-emerald-100'}`}
                                                        title={template.is_active !== false ? 'Pause' : 'Activate'}
                                                    >
                                                        {actionLoading === template.id ? <Loader2 size={18} className="animate-spin" /> : (template.is_active !== false ? <Pause size={18} /> : <Play size={18} />)}
                                                    </button>
                                                    <button 
                                                        ...
                                                        title="Delete"
                                                    >
                                                        {actionLoading === template.id ? <Loader2 size={18} className="animate-spin" /> : <Trash2 size={18} />}
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
                ...
                <div className="px-10 py-6 bg-slate-50/50 border-t border-slate-100 flex items-center gap-10">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Automation Template</span>
                    </div>
